...
typedef struct CryptEngineData
{
   ...
   uint32_t unicv_page; // page is used as salt to derive keys
   uint16_t type;
   uint16_t pmi_bcl_flag;
   uint16_t key_id; // used for WithKeygen F00D functions. always 0.
   uint16_t flag0;
...
{
   uint32_t opt_code; // if 3 then decrypt, if 4 then encrypt, if 2 then encrypt
   CryptEngineData* data;
   
   unsigned char* unk_10; // unknown but probably pointer
   uint32_t unk_18; // unknown but probably size (based on tweak key derrivation)
   uint32_t nBlocksTail;
   
   uint32_t nBlocks; // number of file sectors corresponding to unicv page with signatures
   
   uint32_t sector_base; // first file sector corresponding to unicv page with signatures. used to derive iv for xts-aes
   uint32_t dest_offset; // not sure
   
   uint32_t tail_size; // size of last sector corresponding to unicv page with signatures. should be equal to file sector size in case of full page.
...
{
   uint32_t unk_40; // unknown
   uint32_t sceiftbl_version; // version of unicv
